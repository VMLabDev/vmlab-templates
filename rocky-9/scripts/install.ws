// Build provision for the rocky-9 template. The GenericCloud image ships
// the guest agent preinstalled but with guest-exec blocked (EL default);
// cloud-init's runcmd clears the filter and restarts the agent. So the
// agent answers pings early while exec only starts working mid-runcmd —
// poll exec until it goes through, then block on `cloud-init status
// --wait` so the image is only sealed after first-boot configuration
...
fn main(lab: Lab) {
    ...
    match provision(lab) {
        ...
        Err(e) => {
            ...
            failed.expect("rocky-9 build failed: " + e)
        },
    }
}
