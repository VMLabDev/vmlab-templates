// Build provision for the windows-server-2008-core template (PRD §6.1, §10.4). Legacy Windows
...
fn install(lab: Lab) -> Result[unit, string] {
    ...
    lab.log("installing Windows Server 2008 (x86) Server Core + sysprep; the answer file powers off when done (20-50 min)...")
...
fn main(lab: Lab) {
    install(lab).expect("windows-server-2008-core build failed")
}
