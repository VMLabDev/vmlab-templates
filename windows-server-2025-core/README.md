# Windows Server 2025 (Evaluation, Server Core) template

Builds `x86_64/windows-server-2025-core` into the local template store from
...
Same ISO, same answer file and same build as `windows-server-2025/`, with one
...
vmlab template list      # → x86_64/windows-server-2025-core@26100.1742
```
...
- The sealed image is **generalized**: each clone's first boot replays
  ...
  VMLAB-WS2025C is replaced per clone). First boot of a clone takes a few extra
