# Windows Server 2022 (Evaluation, Server Core) template

Builds `x86_64/windows-server-2022-core` into the local template store from
...
Same ISO, same answer file and same build as `windows-server-2022/`, with one
...
vmlab template list      # → x86_64/windows-server-2022-core@20348.169
```
...
- The sealed image is **generalized**: each clone's first boot replays
  ...
  VMLAB-WS2022C is replaced per clone). First boot of a clone takes a few extra
