// Build provision for the fedora-44 template. Fedora/EL block guest-exec
// in the agent by default; cloud-init's runcmd clears the filter and
// restarts the agent. So the agent may answer pings while exec is still
// blocked — poll exec until it goes through, then block on `cloud-init
// status --wait` so the image is only sealed after first-boot
...
fn main(lab: Lab) {
    ...
    match provision(lab) {
        ...
        Err(e) => {
            ...
            failed.expect("fedora-44 build failed: " + e)
        },
    }
}
